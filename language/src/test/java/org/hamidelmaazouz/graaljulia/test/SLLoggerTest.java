...
public class SLLoggerTest {
    ...
    public void testLoggerSlFunctionLevelFine() {
        final Context context = createContext(createLoggingOptions("sl", "org.hamidelmaazouz.graaljulia.runtime.SLFunction", "FINE"));
        executeSlScript(context);
    ...
    public void testLoggerSlFunctionParentLevelFine() {
        final Context context = createContext(createLoggingOptions("sl", "org.hamidelmaazouz.graaljulia.runtime", "FINE"));
        executeSlScript(context);
        Assert.assertFalse(functionNames(testHandler.getRecords()).isEmpty());
    }
    ...
    public void testLoggerSlFunctionSiblingLevelFine() {
        final Context context = createContext(createLoggingOptions("sl", "org.hamidelmaazouz.graaljulia.runtime.SLContext", "FINE"));
        executeSlScript(context);
        Assert.assertTrue(functionNames(testHandler.getRecords()).isEmpty());
    }
    ...
    public void testMultipleContextsExclusiveFineLevel() {
        ...
        try (Context ctx = Context.newBuilder("sl").options(createLoggingOptions("sl", "org.hamidelmaazouz.graaljulia.runtime.SLFunction", "FINE")).logHandler(handler1).build()) {
            executeSlScript(ctx, ADD_SL, 2);
        }
        final TestHandler handler2 = new TestHandler();
        try (Context ctx = Context.newBuilder("sl").options(createLoggingOptions("sl", "org.hamidelmaazouz.graaljulia.runtime.SLFunction", "FINE")).logHandler(handler2).build()) {
            executeSlScript(ctx, MUL_SL, 1);
        }
        final TestHandler handler3 = new TestHandler();
        try (Context ctx = Context.newBuilder("sl").options(createLoggingOptions("sl", "org.hamidelmaazouz.graaljulia.runtime.SLFunction", "FINE")).logHandler(handler3).build()) {
            executeSlScript(ctx, ADD_SL, 2);
        }
    ...
    public void testMultipleContextsNestedFineLevel() {
        ...
        final TestHandler handler2 = new TestHandler();
        final TestHandler handler3 = new TestHandler();
        try (Context ctx1 = Context.newBuilder("sl").options(createLoggingOptions("sl", "org.hamidelmaazouz.graaljulia.runtime.SLFunction", "FINE")).logHandler(handler1).build()) {
            try (Context ctx2 = Context.newBuilder("sl").options(createLoggingOptions("sl", "org.hamidelmaazouz.graaljulia.runtime.SLFunction", "FINE")).logHandler(handler2).build()) {
                try (Context ctx3 = Context.newBuilder("sl").options(createLoggingOptions("sl", "org.hamidelmaazouz.graaljulia.runtime.SLFunction", "FINE")).logHandler(handler3).build()) {
    ...
    private static Set<String> functionNames(final List<? extends LogRecord> records) {
        return records.stream().filter((lr) -> "sl.org.hamidelmaazouz.graaljulia.runtime.SLFunction".equals(lr.getLoggerName())).map((lr) -> (String) lr.getParameters()[0]).collect(Collectors.toSet());
    }
